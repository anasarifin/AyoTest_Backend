...
module.exports = {
	...
	detailAdmin: id => {
		return new Promise((resolve, reject) => {
			conn.query(`SELECT * FROM admin WHERE deleted = '0' AND id_admin = '${id}'`, (err, result) => {
				if (!err) {
					resolve(result);
				} else {
					reject(err);
	...
	register: (data, img) => {
		return new Promise((resolve, reject) => {
			conn.query("select email from admin where email = ?", data.email, (err, result) => {
				if (result.length < 1) {
					conn.query("insert into admin set name = ?, password = ?, email = ?, picture = ?, gender = ?, phone = ?, address = ?, deleted = ?", [data.name, data.hash, data.email, data.image, data.gender, data.phone, data.address, data.deleted], (err, res) => {
						if (!err) {
	...
	updateAdmin: (data, id_admin) => {
		return new Promise((resolve, reject) => {
			if (data.image) {
				conn.query(`select * from admin where id_admin = ${id_admin}`, (err, result) => {
					fs.unlink("uploads/" + result[0].picture, () => resolve(err));
				});
				conn.query("update admin set name = ?, gender = ?, email = ?, phone = ?, picture = ?, address = ?, where id_admin = ?", [data.name, data.gender, data.email, data.phone, data.image, id_admin], (err, res) => {
					if (!err) {
						resolve(res);
					} else {
						reject(err);
